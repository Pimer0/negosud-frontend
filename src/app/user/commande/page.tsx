...
export default function CommandeFournisseur() {
  // mise en place des onglets pour la création et la gestion de commande
  const [activeTab, setActiveTab] = useState<'creation' | 'gestion'>('creation');
  
  // states pour la création de commande
  ...
  // states pour la gestion des commandes
  ...
  // on récupère la liste des fournisseurs au chargement de la page
  useEffect(() => {
  ...
  // récupère la liste des articles du fournisseur choisi
  const fetchArticles = async (fournisseurId: number) => {
    try {
      ...
      if (result.success) {
        ...
        // on trouve le nom du fournisseur
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        utilisateurId: session.UserId ? parseInt(session.UserId) : 1,
      ...
      if (result.success) {
        ...
        setOrderLines([]);
        // on bascule vers l'onglet de gestion après une création réussie
